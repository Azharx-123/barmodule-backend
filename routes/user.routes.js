...
router.get("/profile", authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId)
      .select("-password -__v")
      .populate("enrolledCourses.courseId", "title slug image");
...
router.put("/progress/:courseId", authMiddleware, async (req, res) => {
  try {
    ...
    if (typeof progress !== "number" || Number.isNaN(progress)) {
      return res.status(400).json({ message: "Progress harus berupa angka" });
    }
    const clampedProgress = Math.min(100, Math.max(0, progress));

    const user = await User.findById(req.user.userId);
    const courseIndex = user.enrolledCourses.findIndex(
      (c) => c.courseId && c.courseId.toString() === courseId, // ⬅️ tambahan guard null
    );
    ...
    user.enrolledCourses[courseIndex].progress = clampedProgress; // ⬅️ pakai versi yang sudah di-clamp
    await user.save();
